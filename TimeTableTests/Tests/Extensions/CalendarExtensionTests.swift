...
class CalendarExtensionTests: XCTestCase {
    ...
    func testDateByAddingComponentsToDate() throws {
        ...
        let date = try XCTUnwrap(DateComponents(calendar: Calendar(identifier: .iso8601), hour: 23).date)
    ...
    func testDateByAddingComponentsWithIntValueToDate() throws {
        ...
        let date = try XCTUnwrap(DateComponents(calendar: Calendar(identifier: .iso8601), month: 1, day: 31).date)
